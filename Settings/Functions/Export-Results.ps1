function Export-Results {
    ...
    try {
        switch ($Format) {
            'CSV'  { ($data | Select-Object Index, FileName, @{Name='Type';Expression={$_.Kind}}, LineNumber, @{Name='Content';Expression={$_.Line}}, Path) | Export-Csv -Path $path -Encoding UTF8 -Delimiter ';' -NoTypeInformation -Force }
            'JSON' { (($data | Select-Object Index, FileName, @{Name='Type';Expression={$_.Kind}}, LineNumber, @{Name='Content';Expression={$_.Line}}, Path) | ConvertTo-Json -Depth 4) | Set-Content -Path $path -Encoding UTF8 }
